...
| Custom RUDP | protocol/unit layer와 Reliable Ordered event queue 검증 | 실제 UDP gameplay transport 전환 |
| Room Actor / WorkerPool | primitive/regression과 TCP inline actor pump | production WorkerPool thread 연결 |
| Observability / stress | metric primitive와 테스트 baseline | 100-client stress/soak 결과 기록 |
...
- RUDP는 protocol layer와 event queue를 검증했지만, 실제 gameplay transport는 아직 TCP 기준입니다.
...
3. RUDP `InputCommand -> RoomEvent` dispatch와 outbound transport policy를 정합니다.
