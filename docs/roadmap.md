...
1. Release1 capacity run artifact 중 public에 공개 가능한 요약만 선별해 `docs/test_matrix.md`에 연결합니다.
2. RUDP gameplay result surface를 넓히고, loss/jitter/reorder 기준의 soak evidence를 추가합니다.
3. production WorkerPool 연결 범위를 명확히 정하고, Room ownership policy를 문서와 테스트로 고정합니다.
4. Meta Server와 C++ server의 runtime settlement 연동 근거를 추가합니다.
5. 데모 영상이나 GIF를 만들면 README의 demo build 링크 아래에 짧게 연결합니다.
...
- 실행 파일, 영상, GIF는 링크가 실제로 준비된 경우에만 README에 추가합니다.
