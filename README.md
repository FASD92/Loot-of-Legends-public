...
> 클라이언트 입력을 신뢰하지 않고, **서버가 Room 상태와 루팅 소유권을 최종 판정**하는 C++17 실시간 게임 서버 포트폴리오 프로젝트입니다.
...
| Spring Meta Server / MySQL / Redis | 예정 | `SettlementResult`를 영속 자산으로 반영하는 정산 세로 슬라이스 구현 예정 |
| Custom RUDP | 예정 | TCP로 검증한 애플리케이션 프로토콜을 RUDP 전송 계층으로 확장 예정 |
| Unity Thin Client | 예정 | 서버 응답을 표시하는 얇은 렌더러로 구현 예정 |
| 100-client stress / soak | 예정 | Room actor / WorkerPool 전환 이후 운영 검증 예정 |

현재 공개 레포의 초점은 **C++ TCP 서버 권한 루프 + Debug CLI + 자동 테스트**입니다.
...
| 테스트 가능한 설계 | RoomManager 단위 테스트와 TCP 통합 테스트로 핵심 불변식 검증 |
...
flowchart TD
    ...
    subgraph FutureMeta["Future Meta Boundary"]
    ...
    SettlementCache -. "Phase 3 예정" .-> SpringMeta
```

현재 구현은 TCP 기반 단일 서버 프로세스에서 Room 도메인 불변식을 먼저 고정하는 단계입니다. 이후 단계에서는 같은 애플리케이션 메시지 구조를 유지하면서 Custom RUDP, Room Actor, WorkerPool, Meta/DB 정산으로 확장합니다.
...
> 참고: 현재 `SettlementResult` 멱등성은 C++ 서버 프로세스 메모리 기준입니다. 영속 멱등성은 이후 Spring Meta Server + MySQL `settlementId` unique key + transaction으로 확장할 예정입니다.
...
`MetaResponse`는 아직 실제 Spring Meta Server 연동 전 단계의 계약용 packet입니다. 이후 Meta/DB 정산 세로 슬라이스가 붙으면 `SettlementResult` 처리 결과를 서버로 되돌려주는 경계로 사용합니다.
...
| `protocol/*` | 향후 프로토콜 계층 확장을 위한 parser/serializer/dispatcher skeleton |
...
| Phase 3 | 예정 | Spring Meta Server + MySQL/Redis 기반 멱등 정산 세로 슬라이스 |
| Phase 4 | 예정 | Custom RUDP, Reliable Ordered / Unreliable Sequenced 전송 계층 |
...
1. `SettlementResult`를 Spring Meta Server로 전송합니다.
2. MySQL `settlement_records`의 `settlementId` unique key와 transaction으로 중복 정산을 방지합니다.
3. Redis session token 또는 internal credential 검증 경계를 둡니다.
4. 전투/루팅 hot path에는 동기 DB 접근을 넣지 않고, DB는 세션 종료 정산 경계에서만 연결합니다.
5. 이후 Room Actor / WorkerPool로 동일 Room 직렬 처리와 다중 Room 병렬 처리를 분리합니다.
...
| DB 정산 | 현재는 C++ 서버의 `SettlementResult` 생성까지 구현되어 있습니다. 영속 반영은 Phase 3에서 구현 예정입니다.              |
| 멱등성 범위 | 현재 반복 `finish_session` 멱등성은 서버 프로세스 메모리 기준입니다. DB unique key 기반 영속 멱등성은 예정입니다.         |
| 전송 계층 | 현재는 TCP입니다. Custom RUDP는 이후 같은 application message contract 위에 올릴 예정입니다.               |
